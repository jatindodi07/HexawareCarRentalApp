	...
	SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
		http
		 ...
		 .authorizeHttpRequests(authorize -> authorize
				 	...
				 	.requestMatchers(HttpMethod.POST, "/api/signup/{location_id}").permitAll() 
					...
					.requestMatchers(HttpMethod.POST, "/api/add/rental-price/{id}").hasAuthority("EXECUTIVE") 
					.requestMatchers(HttpMethod.GET, "/fetch/rental/price/{id}").authenticated()
					.requestMatchers(HttpMethod.POST, "/add/booking/{user_id}/{car_id}").authenticated()
